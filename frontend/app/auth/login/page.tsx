'use client';

import { Suspense } from 'react';
import { useSearchParams } from 'next/navigation';
import Link from 'next/link';
import { motion } from 'framer-motion';
import { BRAND } from '@/lib/constants';
import BrandLogo from '@/components/BrandLogo';
import { getGoogleAuthUrl, getOAuthCallbackUrl } from '@/lib/auth';

function LoginContent() {
  const searchParams = useSearchParams();
  const redirect = searchParams.get('redirect') || '/products';
  const error = searchParams.get('error');

  const googleUrl = getGoogleAuthUrl(getOAuthCallbackUrl(redirect));

  return (
    <div className="container-main flex min-h-[70vh] items-center justify-center py-16">
      <motion.div
        initial={{ opacity: 0, y: 30, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 0.6 }}
        className="surface-card w-full max-w-md p-10 text-center shadow-card"
      >
        <div className="flex flex-col items-center">
          <BrandLogo size="xl" showName={false} href="/" priority />
          <p className="mt-4 font-display text-2xl font-black tracking-tight">{BRAND.name}</p>
          <p className="mt-1 text-sm text-store-muted">{BRAND.tagline}</p>
        </div>

        <h1 className="mt-8 text-xl font-bold">Sign in to continue</h1>
        <p className="mt-2 text-sm text-store-muted">
          Login is required for cart, checkout, and orders. Browse freely without an account.
        </p>

        {error && (
          <p className="mt-4 rounded-xl bg-store-faint px-4 py-2 text-sm text-red-500">
            Sign in failed. Please try again.
          </p>
        )}

        <motion.a
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          href={googleUrl}
          className="btn-primary mt-8 flex w-full items-center justify-center gap-3"
        >
          <svg className="h-5 w-5" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="currentColor"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="currentColor"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="currentColor"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Continue with Google
        </motion.a>

        <Link href="/products" className="mt-6 inline-block text-sm text-store-muted hover:text-store-text">
          ← Back to shopping
        </Link>
      </motion.div>
    </div>
  );
}

export default function LoginPage() {
  return (
    <Suspense fallback={<div className="container-main py-20 text-center">Loading...</div>}>
      <LoginContent />
    </Suspense>
  );
}
